...
除了主要的代码行统计功能外，项目还包含以下辅助工具：

### 文件扩展名统计工具

`test/scripts/extension_counter.py` 是一个Python脚本，用于在测试阶段帮助识别目录中的所有文件类型。该工具用于：

- 检查Ruby代码统计时有哪些非常规Ruby文件被程序疏漏
- 统计目录及其子目录中的所有文件扩展名及数量
- 列出无扩展名的文件路径
...
无扩展名文件的路径:
--------------------------------------------------
/path/to/directory/Rakefile
/path/to/directory/Gemfile
...
--------------------------------------------------
共有 7 个无扩展名文件
```
...
项目包含用于测试Ruby代码统计功能的脚本test-ruby-count.bat

- 该脚本于Windows平台开发和使用： `test/scripts/test-ruby-count.bat`

该脚本用于比较本工具与标准cloc工具对Ruby文件的统计结果，确保统计的一致性和准确性。
